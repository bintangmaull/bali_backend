...
def log_metadata_to_db(filename: str, storage_path: str, public_url: str):
    ...
    """
    print(f"📝 Logging metadata to database for {filename}...")
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        cur.execute("""
...
def reproject_to_wgs84(src_path: str, tmp_dst_path: str):
    ...
    """
    dst_crs = 'EPSG:4326'
    print(f"🌍 Reprojecting {src_path} to {dst_crs}...")
    ...
    print(f"✅ Reprojection complete: {tmp_dst_path}")
...
def convert_to_cog(src_path: str, dst_path: str):
    ...
    print(f"✅ COG created at {dst_path}")
...
def ensure_storage_bucket():
    ...
    print(f"📦 Checking Supabase bucket '{STORAGE_BUCKET}'...")
    try:
...
def upload_to_supabase(file_path: str, destination_name: str):
    ...
    """
    if not SUPABASE_URL or not SUPABASE_KEY or "PASTE_YOUR" in SUPABASE_KEY:
        print(f"❌ Skipping upload for {destination_name}: Missing or placeholder SUPABASE_SERVICE_ROLE_KEY.")
        return None

    print(f"🚀 Uploading {file_path} to Supabase bucket '{STORAGE_BUCKET}' as '{destination_name}'...")
    ...
    try:
        ...
        # Let's check if it's an error response (some versions return a dict or object with 'error')
        if isinstance(response, dict) and 'error' in response:
            print(f"❌ Upload failed for {destination_name}: {response['error']}")
            return None
            
        public_url = f"{SUPABASE_URL}/storage/v1/object/public/{STORAGE_BUCKET}/{destination_name}"
        print(f"✅ Upload successful for {destination_name}")
        return public_url
    except Exception as e:
        print(f"❌ Upload error for {destination_name}: {e}")
        return None

def process_file(input_path: Path, base_dir: Path | None = None):
    """
    Process a single GeoTIFF: Convert to COG, upload, and log to DB.
    """
    try:
        # Determine destination name for Supabase (preserving subfolder structure)
        if base_dir:
            relative_path = input_path.relative_to(base_dir)
            # Replace local path separators with forward slash for storage
        ...
        # 5. Clean up temporary and cog files (Optional - keeping cog for now)
        if tmp_reprojected.exists():
            os.remove(tmp_reprojected)
            print(f"🧹 Cleaned up temporary reprojected file: {tmp_reprojected}")
...
def main():
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not verify/create database or storage resources: {e}")

    if input_path.is_dir():
        print(f"📂 Scanning directory: {input_path}")
        # Find all .tif and .tiff files recursively
        files = list(input_path.rglob("*.tif")) + list(input_path.rglob("*.tiff"))
        # Filter out files that already start with 'cog_' to avoid re-processing
