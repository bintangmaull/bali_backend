# app/service/service_kurva_banjir_rc.py
...
from app.models.models_database import HasilProsesBanjirRC
...
def process_data(input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    Untuk setiap baris input_data:
      - ambil referensi kurva tipe '1' & '2' (jumlah lantai) dari referensi_dmgratio_banjir
      - interpolasi rc_25, rc_50, rc_100, rc_250
      - hasilkan kolom dmgratio_1_rc* dan dmgratio_2_rc*
    Input DataFrame harus berisi kolom: id_lokasi, rc_25, rc_50, rc_100, rc_250
    """
    logger.info("📥 Memulai proses interpolasi Banjir RC...")
    ...
    for tipe in ['1', '2']:
        for s in RC_SCALES:
            df[f'dmgratio_{tipe}_rc{s}'] = None
    ...
    for tipe, ref in reference_curves.items():
        ...
        if not x_ref or not y_ref:
            logger.warning(f"⚠️ Referensi tipe {tipe} kosong: seluruh dmgratio_{tipe}_rc* = None")
            continue
        ...
        for s in RC_SCALES:
            in_col  = f'rc_{s}'
            out_col = f'dmgratio_{tipe}_rc{s}'
    ...
    cols = ['id_lokasi'] + [
        f'dmgratio_{t}_rc{s}'
        for t in ['1', '2'] for s in RC_SCALES
    ]
    result = df[cols].copy()
    ...
    # result sudah terisi di loop sebelumnya.
